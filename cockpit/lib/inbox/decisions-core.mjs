...
//
// Pure decision logic shared by the three decision API routes
// (app/api/sessions/[id]/{approve,redispatch,reject}/route.ts). Zero I/O so
// it can be unit-tested against fixtures with no live Supabase project (see
// decisions-core.test.mjs). Encodes the status-transition table from
// docs/SCHEMA_V2.md's "operator-driven" section:
