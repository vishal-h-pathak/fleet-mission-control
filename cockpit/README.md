...
- `proxy.ts` — runs on every route except `/login` and `/auth/callback`:
...
- `lib/auth/allowlist.mjs` — the pure, framework-free email-compare function
  proxy.ts uses (comma-separated / trimmed / case-insensitive / exact
...
- `lib/supabase/admin.ts` — **service-role** client, guarded by
  ...
  import it from proxy.ts or any `"use client"` module.
...
cookie so `proxy.ts` (edge) and Server Components can read the same
...
| `COCKPIT_ALLOWED_EMAILS` | server-only | Comma-separated allowlist proxy.ts checks against. |
...
clean, and `/login` renders. The magic-link round trip and the proxy's
